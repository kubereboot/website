...
```bash
# while writing latest is 1.22.0
latest=$(curl -s https://api.github.com/repos/kubereboot/kured/releases | jq -r '.[0].tag_name')
wget "https://github.com/kubereboot/kured/releases/download/$latest/kured-$latest-combined.yaml"
```

In the `kured-1.22.0-combined.yaml` the daemonset has the following tolerations:
...
After applying the `kured-1.22.0-combined.yaml` file you can check the number of pods with:
