...
serve: ## Spawns a development server.
	hugo server \
...
production-build: ## Builds production build.
	hugo \
		--baseURL $(URL) \
		--gc \
		--minify \
...
preview-build: ## Builds a preview build (for e.g. a pull requests).
	hugo \
...
branch-build: ## Builds a Git branch (for e.g. development branches).
	hugo \
		--baseURL $(DEPLOY_PRIME_URL) \
		--buildDrafts \
		--buildFuture \
